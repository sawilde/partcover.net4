...
public:
	MethodSigWriter(std::wstring* value, IMetaDataImport* md) : _value(value), _md(md) {}

protected:
	void Append(LPCWSTR w) { *_value += w; }
	void Append(const std::wstring& w) { *_value += w; }
	void Append(const unsigned int& i) { 
		WCHAR buffer[10];
		swprintf_s(buffer, 10, L"%d", i);
		Append(buffer); 
	}


	virtual void NotifyByref() { Append(L"ref ");}
	virtual void NotifyVoid() { Append(L"void"); }
	virtual void NotifyEndRetType() { Append(L" "); }

	virtual void NotifySentinal() { Append(L" ..."); }
	virtual void NotifyBeginMethodParamList() { Append(L" ("); }
	virtual void NotifyHasMoreParameters() { Append(L", "); }
	virtual void NotifyEndMethodParamList() { Append(L")"); }

	virtual void NotifyTypeDefOrRef(mdToken token);
	virtual void NotifyTypeSimple(sig_elem_type elem_type);

	virtual void NotifyGenericParamCount(sig_count count) { Append(L"<"); Append(count); Append(L">"); }

	virtual void NotifyCustomMod(sig_elem_type cmod, mdToken token) { Append(L"const "); }

		// starting array shape information for array types
	virtual void NotifyBeginArrayShape() { Append(L"["); }
	virtual void NotifyEndArrayShape() { Append(L"]"); }

	// array rank (total number of dimensions)
	virtual void NotifyRank(sig_count count) { Append(count); Append(L":"); }

	// number of dimensions with specified sizes followed by the size of each
	virtual void NotifyNumSizes(sig_count count) {}
	virtual void NotifyNextSize() { Append(L" "); };
	virtual void NotifySize(sig_count count) {}

	// BUG BUG lower bounds can be negative, how can this be encoded?
	// number of dimensions with specified lower bounds followed by lower bound of each 
	virtual void NotifyNumLoBounds(sig_count count) { Append(L":"); }  
	virtual void NotifyNextLoBound() { Append(L" "); };
	virtual void NotifyLoBound(sig_count count) {}

	virtual void NotifyTypeSzArray() { Append(L"[]"); }
};