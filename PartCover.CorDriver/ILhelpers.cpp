...
#define IsSmallBranch(ilop) ((ilop).IsBranch() && NULL != strstr((ilop).mnemonic, ".s"))
...
#define OPDEF(name, str, decs, incs, args, optp, stdlen, stdop1, stdop2, flow) \
            { \
            ILop ilop = {IlopDefaultPos, name, str, stdop1, stdop2, stdlen, args, flow, decs, incs}; \
...
#undef OPDEF
        ...
        ILop FindILOpByMnemonic(const char* mnemonic) {
            ILopMap::iterator mapIt = m_map.begin();
            while(mapIt != m_map.end()) {
                ILop& ilop = (mapIt++)->second;
